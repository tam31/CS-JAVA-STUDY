...
- 동시에 PCB가 메모리 커널영역에 생성이 됩니다.
  ...
  - 각각의 스레드는 프로세스의 공유자원(데이터영역,코드영역,힙영역)을 공유하고 있기 때문에 협력에 유리합니다.
     ...
     - 멀티 프로세스 : 똑같은 작업을 수행하는 단일 스레드 프로세스 여러개 실행
     - 멀티 스레드 : 하나의 프로세스에 여러개의 스레드 실행
     - 멀티 스레드의 경우 프로세스의 메모리 영역을 공유하므로 통신비용이 적다 (응답시간 단축)
     - 멀티 프로세스보다 멀티 스레드가 적은 메모리 공간을 차지하며 문맥교환이 빠르므로 속도가 더 빠르다.
     ### Q-3) 크롬의 탭은 프로세스인가 스레드인가
     - 크롬의 탭은 탭끼리 영향을 받지 않기 떄문에 프로세스입니다. 하나의 탭을 강제종료 해도 다른 탭들에 영향을 받지 않습니다. 따라서 크롬 브라우저는 멀티 프로세스 프로그램입니다.

  ### Q) 프로세스와 스레드는 각각 어떻게 생성
  - 보조기억 장치에 있는 프로그램이 실행시 메모리에 프로세스가 적재되고 PCB가 커널영역에 생성이 됩니다. 프로세스가 생성될 때 프로세스 내에 기본스레드 1개도 같이 생성됩니다.
  ### Q) 프로세스 메모리 구조 설명
  - 프로세스의 메모리 구조는 총 4가지로 이뤄져있습니다.
  - 코드 영역 : 실행 명령 코드들의 영역 , 프로그램 시작 시 고정되고 변경이 불가능한 영역(read-only)
  - 데이터 영역 : 프로그램이 실행되는 동안 유지할 데이터를 저장하는 영역 (전역변수들이 여기에 해당됨)
  - 힙 영역 : 프로그래머가 직접 할당할 수 있는 저장공간, 자바 new 연산자로 객체 생성시 힙영역에 생성
    - 언젠가 메모리르 반환해줘야하는 영역 -> 현재 대부분의 프로그래밍 언어는 GC가 이 역할을 하고 있다.
  - 스택 영역 : 데이터가 일시적으로 저장되는 공간 , 잠깐 쓰는 데이터를 저장 ( 매개변수 , 지역변수 )
  - 스택 영역은 높은 주소에서 낮은 주소로 , 힙 영역은 낮은 주소에서 높은 주소로 할당된다. 
      ### Q-1) 초기화 하지 않은 변수들
      - 초기화 되지 않은 변수들은 데이터영역 안에 BSS 영역에 저장이 된다.
      ### Q-2) 동적 라이브러리 접근
      - 답변 내용
      ### Q-3) Stack Size는 언제 결정되냐?
      - 프로세스가 메모리에 로드될 때 사이즈가 고정적으로 결정되며 런타임 상황에서 변경할 수 없다.
      - 지정된 스택 영역을 넘어가버리면 스택오버플로우가 발생된다.
      ### Q-4) 왜 Stack이 heap 보다 빠르냐?
       - 스택은 이미 할당된 공간을 사용하는 것이고, 힙은 따로 할당해서 사용해야 하기 때문
       - 스택은 의미 그대로 차례대로 쌓기 떄문에 연속적 메모리 공간으로 접근이 빠르다.

  ### Q) 프로세스 상태에는 어떤 것들이 있나요?
  - 대부분 운영체제는 5가지 상태를 가지고 있다.
  - 생성 : 이제 메모리에 적재되어 PCB를 할당받은 상태, 준비가 완료되면 준비상태로 변경됨
  - 준비 : 당장 CPU할당을 받을 수 있는 상태, 자신의 차례 때 실행상태로 변경된다(디스패치)
  - 실행 : cpu할당을 받아서 실행되고 있는 상태, 시간 모두 사용시 타이머 인터럽트 발생하여 준비 상태로 이동
    - 실행 도중 입출력 장치를 사용하면 입출력 작업이 끝날 때까지 대기상태로 변경
  - 대기 : 프로세스가 입출력 작업을 하는 과정, 입출력 장치는 CPU보다 느리기에 대기상태로 변경된다. 입출력이 끝나면 준비상태로 변경
  - 종료 : 프로세스 종료를 의미, PCB와 프로세스 메모리 영역이 정리된다.
      #### Q-1) non-preemptive 에서 있을 수가 없는 상태 변화
      - 비선점 스케줄링의 경우 하나의 프로세스가 끝날 때 까지 대기해야하기 때문에 실행상태에서 준비상태로 가는 상태변화가 나타나지 않는다.

      #### Q-2) Memory가 부족할 경우, Process는 어떠한 상태로 변하는가
      - 메모리가 부족하면 OutOfMemory상태가 되므로 kill할 프로세스를 찾아서 강제 종료한다.

  ### Q) 좀비 프로세스, 고아 프로세스
  - 고아 프로세스 : 부모 프로세스가 자식 프로세스보다 먼저 종료된 경우 자식 프로세스를 고아 프로세스라고 한다.
  - 좀비 프로세스 : 부모 프로세스가 wait 함수를 호출하지 않아 최소한의 정보 메모리에 남아 있는 프로세스
---
