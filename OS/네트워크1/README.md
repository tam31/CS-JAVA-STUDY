...
- 200 (OK)는 클라이언트의 요청이 성공적으로 처리되었고 ,서버가 클라이언트에게 요청한 데이터나 리소스를 제공함을 나타냅니다. 응답코드는 주로 GET 요청에 사용되며, 요청된 데이터가 성공적으로 반환되었음을 의미합니다.
...
- TCP는 연속성보다 신뢰성 있는 전송이 중요할 때 사용되는 프로토콜이며, UDP는 TCP보다 빠르고 네트워크 부하가 적다는 장점이 있지만, 신뢰성 있는 데이터 전송을 보장하지는 않는다. 그래서 UDP는 신뢰성보다는 연속성이 중요한 실시간 스트리밍과 같은 서비스에 자주 사용된다
...
#### checksum
- 네트워크를 통해 전달된 값이 변경되었는지를 검사하는 값으로 무결성을 제공방법입니다.
- 네트워크를 통해 전달되는 값이 중간에 해커를 통해서든 , 네트워크의 오류로 변질되는 경우를 대비하여 체크섬값을 같이 보내어 무결성을 제공하게 된다
#### TCP / UDP 헤더 차이
- TCP는 출발지 도착지 포트와 시퀀스넘버 등 대규모의 데이터 통신에 적합하도록 많은 정보를 담고 있지만 UDP는 소규모의 데이터 통신에 적합하도록 출발지, 도착지, 전체헤더의 크기, 체크섬 등 8바이트로 고정되어 있다.
#### UDP의 실시간 스트리밍 외 다른 예시
- 실시간성 보장 또는 빠른 연결이 필요한 서비스에 이용되어와서 온라인게임이나 iptv 비디오 스트리밍 같은 곳에서 사용한다고 합니다.

#### 왜 HTTP는 TCP를 사용하나요?
- HTTP는 신뢰성을 보장하지 않기 때문에 신뢰성을 보장하는 TCP를 사용한다
...
- tcp 현재까지 많이 쓰이고 있고 udp는 아직 안전하지도 않고 실무에서 사용되려면 좀더 시행착오가 더 필요하다고 느끼기 때문에

#### TCP와 UDP 중 어느 프로토콜이 Checksum을 수행할까요?
- UDP로 헤더의 Checksum을 통해서(옵션) 헤더와 데이터 모두 포함한 사용자 데이터그램 전체에 대한 오류를 탐지한다.
#### 그렇다면, Checksum을 통해 오류를 정정할 수 있나요?
- 아닙니다. 동작과정을 보면 정보를 보낼 때 송신지에서 계산되고 목적지에서 검사되는데 목적지에서 체크섬 에러 발견 시, 즉시 폐기됩니다.

#### TCP가 신뢰성을 보장하는 방법에 대해 설명해 주세요.
- TCP 재전송으로 TCP는 그 특성상 자신이 보낸 데이터에 대해서 상대방이 받았다는 의미의 응답 패킷을 다시 받아야 통신이 정상적으로 이뤄졌다고 생각합니다.
...
- 혼잡 제어는 네트워크 내의 패킷 수를 조절하여 네트워크의 오버플로(overflow)를 방지하는 기능을 의미합니다.
- AIMD   
...
- 빠른 재전송과 혼잡한 상태가 되면 윈도우 크기를 1로 줄이지 않고 반으로 줄이는 빠른 회복 방법이 있습니다.
